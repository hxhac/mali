...
const getTableData = async() => {
  const table = await getGoodsLabelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteGoodsLabelByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateGoodsLabelFunc = async(row) => {
  const res = await findGoodsLabel({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.regoodsLabel
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteGoodsLabelFunc = async(row) => {
  const res = await deleteGoodsLabel({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
  ...
  switch (type.value) {
    case 'create':
      res = await createGoodsLabel(formData.value)
      break
    case 'update':
      res = await updateGoodsLabel(formData.value)
      break
    default:
      res = await createGoodsLabel(formData.value)
      break
