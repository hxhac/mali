...
func (goodsEvaluationService *GoodsEvaluationService) GetGoodsEvaluationInfoList(info goodsReq.GoodsEvaluationSearch) (err error, list interface{}, total int64) {
	...
	// err = db.Select("goods_evaluation.*", "goods_category.cate_name").Joins("JOIN goods_category ON goods_category.id=goods_evaluation.cate_id").Limit(limit).Offset(offset).Find(&goodsEvaluations).Association("GoodsCategory").Error
	err = db.Preload("GoodsCategory").Limit(limit).Offset(offset).Find(&goodsEvaluations).Error
